...
export class FakeBackendHttpInterceptor implements HttpInterceptor {
    ...
    handleRequests(req: HttpRequest<any>, next: HttpHandler): any {
        ...
        if (url.endsWith('/cows') && method === 'GET') {
            return of(new HttpResponse({status: 200, body: data.result}));
        }

        if (url.includes('/cows') && method === 'DELETE') {
            const cowId = req.params.get('cowId');
            data.result = data.result.filter(cow => cow.cowId !== Number(cowId));
            return of(new HttpResponse({status: 200, body: cowId}));
        }

        if (url.endsWith('/cow') && method === 'POST') {
            const { cow } = req.body;
            data.result = [cow, ...data.result];
            return of(new HttpResponse({status: 200, body: cow }));
        }

        if (url.endsWith('/cow') && method === 'PUT') {
            const { cowInfo } = req.body;
            const cowPosition = data.result.findIndex(({cowId}) => cowId.toString() === cowInfo.cowId);
            if (cowPosition !== -1) {
              const cow =  data.result[cowPosition];
              const updatedCow = {...cow, [cowInfo.key] : cowInfo.value };
              data.result[cowPosition] = updatedCow;
              return of(new HttpResponse({status: 200, body: updatedCow }));
            } else {
                throw Error(`there is no cow with id ${cowInfo.cowId}`);
            }
        }
